...
├── profiles/<name>/
│   ├── profile.toml              ← 身份、简历池、work auth、Q&A
...
| 类别 | 迁移？ |
|---|---|
| Profile 文件（`profile.toml`、`score.md`、`prompts/`、`attachments/`） | **要** — 你的真内容 |
...
# 1. Stable workspace 骨架 — 使用 stable init 流程，这样 wolf.toml 和
#    CLAUDE.md/AGENTS.md 里都是 stable 的 `wolf <verb>` 引用
wolf init
...
wolf init                      # writeIfAbsent 填这两个文件；其他已存在的不动
