...
cd ~/wolf
wolf init
# 选择填写简历内容，按提示输入 summary / education / experience / projects / skills
```
...
- [ ] 询问是否现在填写简历内容
- [ ] 完成后显示 `resume.txt` 的绝对路径，提示可以继续编辑
- [ ] `wolf.toml` 在 `~/wolf/` 生成
- [ ] `data/default_Default/resume.txt` 包含填写的内容
- [ ] `data/` 添加到 `.gitignore`
- [ ] 完成后显示 API key 状态（✓/✗ 各个 key）
- [ ] 提示下一步：`wolf templategen`
...
**检查项：**
- [ ] `data/default_Default/resume.txt` 生成，内容为模板（带 `# SUMMARY` 等占位符）
- [ ] 完成后显示 `resume.txt` 路径，提示编辑后跑 `wolf templategen`
...
`wolf tailor` 依赖 `resume.txt`（内容池）和 `data/<profile>/general_resume/resume.tex`（模板）。

```bash
cd ~/wolf
# 确认 resume.txt 有内容（wolf init 时填写，或手动编辑）
cat data/default_Default/resume.txt
...
> **前置依赖：** 必须先完成步骤 2（wolf templategen），否则会报 `resume.txt not found`。
...
- [ ] 在 `wolf mcp serve` 启动目录下生成 `wolf.toml` 和 `resume.txt`
- [ ] 返回 `resume.txt` 路径和下一步提示
...
- [ ] `data/default_Default/resume.txt` 内容已更新
