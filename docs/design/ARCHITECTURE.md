...
- `AppConfig` — workspace-level config, loaded from `wolf.toml`. Contains the default profile name, command model settings, and companion settings (`servePort`, `maxStagehandSessions`, fixed `browserMode`). Validated at parse time by `AppConfigSchema` (zod). Does **not** embed profile data.
