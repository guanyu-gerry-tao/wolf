...
wolf 分为六层，每层只能依赖其下方的层——不允许横向或向上依赖。
...
Utils（src/utils/）— 横切辅助工具。不属于任何一层。
AppContext（src/cli/appContext.ts）— 手动 DI 容器。
```

**层依赖方向：** `CLI/Commands → Application → Service → Repository → Types`
...
| **仓储层** | `src/repository/` | 读写 SQLite（通过 Drizzle）和工作区文件（`profile.toml`、`resume_pool.md`） | 业务逻辑或调用其他层 |
...
| **命令层** | `src/commands/` | 将类型化参数映射到应用层调用；格式化错误 | 包含业务逻辑——一切委托给应用层 |
| **表现层** | `src/cli/` `src/mcp/` | 解析输入，格式化输出 | 参数映射以外的任何逻辑 |

### 依赖注入——AppContext

所有具体实现都在 `src/cli/appContext.ts` 中构建。其他任何地方都不直接实例化 repository 或 service。这是唯一的替换点：将真实实现换成 mock，只需修改 `appContext.ts`，其他文件一概不变。
...
│   ├── index.ts          # CLI 入口
│   └── appContext.ts     # 手动 DI——连接所有 repo 和 service
├── mcp/              # 表现层 — MCP SDK（暂停，共用同一个 AppContext）
├── commands/         # 薄层封装——每个子命令一个文件夹
├── application/      # 用例编排（接口 + impl/ + model/）
├── service/          # 领域服务（接口 + impl/ + model/）
│   ├── batch.ts          # BatchService 接口
│   ├── jobProvider.ts    # JobProviderService 接口
│   └── impl/
│       ├── apiProviderService.ts   # 真实 HTTP provider
│       └── batchServiceImpl.ts    # Batch API stub（M3+）
├── repository/       # 数据访问（接口 + impl/）
│   ├── job.ts            # JobRepository 接口
│   ├── company.ts        # CompanyRepository 接口
│   ├── batch.ts          # BatchRepository 接口
│   ├── profile.ts        # ProfileRepository 接口
│   └── impl/
│       ├── drizzleDb.ts          # DrizzleDb 类型别名
│       ├── schema.ts             # Drizzle 表定义
│       ├── initializeSchema.ts   # CREATE TABLE IF NOT EXISTS（唯一入口）
│       ├── sqliteJobRepository.ts
│       ├── sqliteCompanyRepository.ts
│       ├── sqliteBatchRepository.ts
│       └── fileProfileRepository.ts  # 从磁盘读取 profiles/<id>/
├── errors/           # 自定义类型错误类
├── types/            # 共享领域类型（横切关注点，不属于某一层）
└── utils/            # 横切辅助工具（logger、config、env、ai）——不属于任何层
...
- `Resume` — 解析后的简历结构。
- `UserProfile` — 每个 profile 的身份信息（姓名、邮箱、移民身份、评分权重、目标岗位/地点）。由 `FileProfileRepository` 从 `profiles/<id>/profile.toml` 加载，由 `UserProfileSchema`（zod）在解析时校验。
...
JD 文本存在 `data/jobs/<dir>/jd.md`（grep 友好、可手工编辑）。tailor 应用服务
通过 `JobRepository.readJdText(jobId)` 读取；SQLite 行只保留结构化元数据。
...
│       ├── profile.toml    # 个人信息 + 评分偏好（由 UserProfileSchema 校验）
│       └── resume_pool.md  # 全量简历内容池（AI 写入 / 用户手改）
...
```
hunt()   ── 写入 → [SQLite: jobs 行 + data/jobs/<dir>/jd.md] ── 读取 → tailor()
...
// fill：读取职位，更新状态 + 截图路径
const job = await jobRepo.get("abc")  // 包含 job.url + job.tailoredResumePdfPath
await jobRepo.update("abc", { status: "applied", screenshotPath: `${outputDir}/screenshot.png` })

// reach：读取职位，写入外联草稿路径
const job = await jobRepo.get("abc")  // 包含 job.companyId, job.title
