...
抽样验证生产打分 prompt 在两种 persona × 三个差异化 JD 上得到的分数区间符
...
# 选三条覆盖整个分数区间的 JD：明确对齐（backend SWE）、边缘（DevOps）、
# 明显错位（数据分析 / 非工程）。CSV 行 id 若变动需要相应更新。
...
- 明显对齐的 backend JD 在 `ng-swe` 下应 ≥ 0.7。
- 明显错位的非工程 JD 在 `ng-swe` 下应 ≤ 0.4。
- 边缘 JD 介于两者之间，且 justification 必须说出具体的摩擦点。
...
记录每个 `Job.score` + `Job.scoreJustification`、persona 名称、JD 行 id，
以及 AI reviewer 的输出。
