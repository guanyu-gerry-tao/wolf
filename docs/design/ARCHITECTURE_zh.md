...
```
┌──────────────────────────────────────────────────────┐
│  表现层  src/cli/index.ts  src/mcp/                   │
│  解析参数、格式化输出。每个 src/cli/commands/<verb>.ts │
│  只是一行委托加一个 formatter。                         │
├──────────────────────────────────────────────────────┤
│  应用层  src/application/                             │
│  用例编排。多步骤流水线。                               │
...
AppContext（src/runtime/appContext.ts）— 手动 DI 容器，CLI 和 MCP 共用。
```

**层依赖方向：** `cli → application → service → repository → utils`
...
| **表现层** | `src/cli/`（`index.ts` + `commands/<verb>.ts`）、`src/mcp/` | 解析输入、格式化输出、承载 inquirer 提示 | 参数映射 + 格式化以外的任何逻辑 |

### 依赖注入——AppContext

所有具体实现都在 `src/runtime/appContext.ts` 中构建。`src/cli/` 和 `src/mcp/` 共用同一个 `AppContext`。其他任何地方都不直接实例化 repository 或 service。这是唯一的替换点：将真实实现换成 mock，只需修改 `appContext.ts`，其他文件一概不变。
...
**职责：**
- 解析命令行参数和 flag
- 调用 `src/application/impl/` 中对应的函数
...
- 将收到的 tool 调用映射到 `src/application/impl/` 中对应的函数
...
### 3. 命令层（`src/application/impl/`）

wolf 的核心。每个文件导出一个 async 函数，包含一个命令的全部业务逻辑。

```
src/application/impl/
...
```typescript
// src/application/impl/add/index.ts — 来自 AI 编排器的单条职位（仅 MCP）
...
// src/application/impl/hunt/index.ts — 只负责接入
...
// src/application/impl/score/index.ts — 只负责处理
